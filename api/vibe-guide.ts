...
export async function getHomeFeed(): Promise<HomeFeed> {
  ...
  return {
    ...
    quickFilters,
    featuredPlaces: places.slice(0, 4),
    dailyOffers: offers,
    todayEvents: todayEvents.slice(0, 2),
    trendingPlaces: [...places].reverse().slice(0, 4),
    nearbyPlaces: places.slice(1, 5),
  };
}
...
export async function getEventTypes() {
  await wait();
  return ['All', 'Live Music', 'DJ', 'Party', 'Special Event', 'Food Event'];
}
